...
suite("Magic Code Task", () => {
	test("Should handle prompt and response correctly", async function () {
		...
		while (Date.now() - startTime < timeout) {
			...
			if (messages.some(({ type, text }) => type === "say" && text?.includes("My name is Magic"))) {
				break
			}
		...
		assert.ok(
			globalThis.provider.messages.some(({ type, text }) => type === "say" && text?.includes("My name is Magic")),
			"Did not receive expected response containing 'My name is Magic'",
		)
	})
})
